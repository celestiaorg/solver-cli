import { jsx as m, Fragment as ye, jsxs as T } from "react/jsx-runtime";
import { AnimatePresence as vt, motion as Ee } from "framer-motion";
import * as gr from "react";
import ut, { Component as Wi, createContext as Zi, useMemo as R, useCallback as ie, useRef as yr, useEffect as X, useState as he, memo as Ji } from "react";
import { useQuery as Ce } from "@tanstack/react-query";
import { cosmoshub as qi, edentestnet as ji, eclipsemainnetAddresses as Yi, eclipsemainnet as Ki, solanamainnetAddresses as Qi, solanamainnet as Hi, chainMetadata as Zo, warpRouteConfigs as Xi, GithubRegistry as $i, PartialRegistry as _i, chainAddresses as ea } from "@hyperlane-xyz/registry";
import Jo, { HTTPError as Kt } from "ky";
import { ChainStatus as ta, TOKEN_COLLATERALIZED_STANDARDS as eo, ProviderType as we, MultiProtocolCore as na, ChainMetadataSchema as ra, mergeChainMetadataMap as to, RpcUrlSchema as oa, WarpCoreConfigSchema as sa, validateZodResult as ia, WarpCore as Vn, MultiProtocolProvider as Lt, TokenAmount as aa, WarpTxCategory as lt } from "@hyperlane-xyz/sdk";
import { ProtocolType as D, isHttpsUrl as ca, isRelativeUrl as ua, normalizeAddress as no, toTitleCase as Wn, objFilter as la, promiseObjAll as da, objMap as ro, tryParseJsonOrYaml as fa, objMerge as ha, toWei as br, assert as ma } from "@hyperlane-xyz/utils";
import ne from "bignumber.js";
...
import { ArrowLeft as Bt, X as qo, Lightning as ya, CheckCircle as ba, Question as so } from "@phosphor-icons/react";
import { Slot as wa } from "@radix-ui/react-slot";
import { cva as xa } from "class-variance-authority";
import { ArrowUpDown as Aa, ChevronDown as an, Timer as Ca, ChevronRight as jo, X as va } from "lucide-react";
import * as io from "@solana/spl-token";
import { Connection as wr, PublicKey as Zn } from "@solana/web3.js";
import { erc20Abi as Ia, isAddress as Ea, createClient as ka, custom as Ta, hexToString as Sa } from "viem";
import { useAccount as Zt, usePublicClient as Ba, useConfig as Na, useWalletClient as Fa, useSwitchChain as Ma } from "wagmi";
import { useWallet as Jt, useConnection as Ra } from "@solana/wallet-adapter-react";
import { useAccount as xr, useOfflineSigners as za, useCosmWasmSigningClient as Ua, useStargateSigningClient as Da, WalletType as de } from "graz";
import * as Ar from "@radix-ui/react-avatar";
import * as ao from "@radix-ui/react-switch";
import * as Pa from "@radix-ui/react-label";
import { bech32 as La } from "bech32";
import { SigningStargateClient as Oa, GasPrice as co } from "@cosmjs/stargate";
import { CosmosNativeSigner as Ga } from "@hyperlane-xyz/cosmos-sdk";
import { BigNumber as Va, logger as Wa } from "ethers";
import Za from "bowser";
import * as Ja from "@radix-ui/react-separator";
import uo from "fuse.js";
import { getClient as qa, createClient as ja, MAINNET_RELAY_API as Ya, configureViemChain as Ka } from "@relayprotocol/relay-sdk";
import { adaptSolanaWallet as Qa } from "@relayprotocol/relay-svm-wallet-adapter";
class Ha extends Wi {
  constructor(t) {
...
const q = {
...
}, Xa = !1, $a = "0.0.0", _a = "https://api.github.com/repos/celestiaorg/hyperlane-ops", ec = "main", tc = "https://proxy.hyperlane.xyz", nc = "", rc = "", oc = JSON.parse(
  "{}"
), sc = "", ic = void 0, We = Object.freeze({
  chainWalletWhitelists: oc,
  ...
  showTipBox: !0,
  version: $a,
  transferBlacklist: rc,
  walletConnectProjectId: nc,
  walletProtocols: [
    D.Ethereum,
    D.Sealevel,
    D.Cosmos
...
}, kn = (e) => e.startsWith("0x") ? parseInt(e, 16).toString() : e, ac = async (e) => {
  try {
    return [await e, null];
  } catch (t) {
    return [null, t];
  ...
  const o = new ne(e), s = new ne(10).pow(-n);
  if (o.isNaN())
    return "";
  if (o.isZero())
    return "0";
...
function cc(e) {
  return !We.shouldDisableChains || !e ? !1 : e.availability?.status === ta.Disabled;
}
function uc(e, t, n = !1) {
  if (!t) return "Unknown";
...
function It(e) {
  return e ? ca(e) ? e : ua(e) ? `${Cr.imgPath}${e}` : null : null;
}
function hc(e, t, n) {
  try {
    ...
    const r = e.getKnownChainNames().reduce((i, u) => (i[u] = {
      ...
      proxyAdmin: "",
      mailbox: ""
    }, i), {}), s = new na(e, r).extractMessageIds(t, n);
    if (s.length) {
...
function gc(e) {
  return pc(e).reduce((n, r) => (n[r.name] = r, n), {});
}
const $t = (e) => new ne(10).exponentiatedBy(e);
function xt(e) {
  let t = e;
...
var cn = /* @__PURE__ */ ((e) => (e.solana = "792703809", e.bitcoin = "8253038", e))(cn || {}), Ko = /* @__PURE__ */ ((e) => (e.EXACT_INPUT = "EXACT_INPUT", e.EXACT_OUTPUT = "EXACT_OUTPUT", e.EXPECTED_OUTPUT = "EXPECTED_OUTPUT", e))(Ko || {});
const bc = "https://api.relay.link";
class ze {
  static ky = Jo.create({
    prefixUrl: bc,
    timeout: !1
  ...
  static async getChains(t = "") {
    try {
      ...
      return n instanceof Kt && n.response ? {
        success: !1,
  ...
  static async getRoute(t) {
    try {
      ...
      return console.error(n), n instanceof Kt && n.response ? {
        success: !1,
        error: (await n.response.json()).message
      } : {
        success: !1,
...
async function Cc(e, t, n) {
  const r = oo.record(ra).safeParse({
    ...wc
  });
  if (!r.success)
  ...
  We.registryUrl ? (q.debug(
    ...
    We.registryUrl
  ), s = await t.getMetadata()) : (q.debug("Using default published registry for chain metadata"), s = Zo), s = la(
    s,
    (y, g) => e.includes(y)
  ), s = await da(
    ro(
  ...
  const i = to(
    s,
    o
  ), u = fa(We.rpcOverrides), d = oo.record(oa).safeParse(
    u.success && u.data
  );
...
async function Ic(e) {
  const t = sa.safeParse(vc), n = ia(t, "warp core typescript config");
  let r;
  try {
    if (We.registryUrl)
      q.debug(
    ...
    q.debug("Using default published registry for warp routes"), r = Xi;
  }
  const s = Object.values(r), i = s.map((g) => g.tokens).flat(), u = s.map((g) => g.options).flat(), d = [...i, ...n.tokens], h = Ec(d), l = [...u, n.options], y = kc(l);
  if (!h.length)
    throw new Error(
...
async function Tc({
  ...
  try {
    const n = await Ic(t), r = Array.from(
      new Set(n.tokens.map((l) => l.chainName))
    ), { chainMetadata: o, chainMetadataWithOverrides: s } = await Cc(r, t), i = new Lt(s), u = Vn.FromConfig(i, n), d = lc(
      u.tokens,
      i
    ), h = Sc(
      n.tokens
...
const Ho = Zi(null), Bc = ({
  ...
  return /* @__PURE__ */ m(Ho.Provider, { value: y, children: e });
}, st = () => {
  const e = ut.useContext(Ho);
  if (!e)
    throw new Error(
...
function Ir(e, t, n) {
  ...
  })).filter((y) => We.showDisabledTokens ? !0 : !y.disabled).map((y) => y.token), { tokens: h } = fc(
    d,
    n
  );
  let l = h.map((y) => ({
...
const Nc = (e) => {
  const { tokens: t } = qt(void 0, e === pe.FAST);
  return t || [];
};
function Nt() {
...
function Fc() {
  ...
  return R(
    () => Object.values(e.metadata).filter(
      (t) => !t.isTestnet && Qo.includes(String(t.chainId))
    ),
    [e.metadata]
  );
}
      ...
      if (n.success) {
        ...
        return {
          chains: n.chains.filter(
            (o) => Qo.includes(o.chainId) && (!r || !r.has(o.chainId))
          ),
          _raw: n._raw
        };
      } else
...
const Jn = (e) => (t) => {
  ...
  Er(
    ...
    }),
    {
      ...
      merge: (e, t) => {
        ...
        if (r) {
          const o = r.from?.chain?.key ? r.from.chain : Tn.from.chain, s = r.to?.chain?.key ? r.to.chain : Tn.to.chain;
          n.state = {
            ...r,
            from: { ...r.from, chain: o },
            to: { ...r.to, chain: s }
...
), $o = ({ message: e, subtext: t, icon: n = null, className: r }) => /* @__PURE__ */ T("div", { className: "text-foreground [&>svg]:text-foreground bg-background-2/40 rounded-md2 mx-auto flex h-auto w-fit flex-none flex-col items-center px-20 py-12 backdrop-blur-sm", children: [
...
] }), fo = ({ message: e, subtext: t, icon: n = null, className: r }) => /* @__PURE__ */ T("div", { className: "text-foreground [&>svg]:text-foreground mt-2 flex flex-col items-center justify-center", children: [
  /* @__PURE__ */ m("div", { className: "bg-secondary flex size-16 items-center justify-center rounded-full [&>svg]:size-6", children: n }),
  /* @__PURE__ */ m(
    "h2",
    {
  ...
  t ? /* @__PURE__ */ m("p", { className: "mt-3 max-w-3xs text-center text-sm", children: t }) : null
] }), _o = ({ fromToken: e }) => /* @__PURE__ */ m("div", { className: "text-xs", children: `1 ${e?.symbol} = 1 ${e?.symbol}` }), Vc = xa(
  ...
  return /* @__PURE__ */ m(
    o ? wa : "button",
    {
      ref: i,
      "data-slot": "button",
      className: se(Vc({ variant: n, size: r, className: t })),
      ...s
    }
  );
});
...
const Wc = ({
  ...
  return /* @__PURE__ */ T(ye, { children: [
    /* @__PURE__ */ T("div", { className: "mb-4 flex items-center gap-2 text-foreground", children: [
      /* @__PURE__ */ m(
        "button",
        {
          type: "button",
    ...
    /* @__PURE__ */ m(
      $o,
      {
        icon: /* @__PURE__ */ m(qo, { className: "size-8 text-red-500" }),
...
var es = { exports: {} }, re = es.exports = {}, Me, Re;
function qn() {
...
var Ze = [], At = !1, dt, Xt = -1;
function qc() {
  !At || !dt || (At = !1, dt.length ? Ze = dt.concat(Ze) : Xt = -1, Ze.length && ns());
}
function ns() {
  if (!At) {
    var e = ts(qc);
...
re.nextTick = function(e) {
  ...
  if (arguments.length > 1)
    for (var n = 1; n < arguments.length; n++)
      t[n - 1] = arguments[n];
  Ze.push(new rs(e, t)), Ze.length === 1 && !At && ts(ns);
};
function rs(e, t) {
...
var jc = es.exports;
const os = /* @__PURE__ */ Zc(jc);
`${os.env.NEXT_PUBLIC_APP_DOMAIN}`;
os.env.NEXT_PUBLIC_WALLET_CONNECT_PROJECT_ID;
const Yc = [
  {
    inputs: [
      {
...
})), Kc = "0x0000000000000000000000000000000000000000", Qc = ({
  chainId: e,
  assets: t,
  chainMetadata: n,
  enabled: r = !1
}) => {
  const { address: o } = Zt(), s = Ba({
    ...
    queryFn: async () => {
      ...
      if (t?.some(
        ...
        return {
          chainId: e,
          balances: {
            "0x43505da95A74Fa577FB9bB0Ce29E293FdF575011": {
              value: g.toString(),
              amount: new ne(g.toString()).dividedBy(
                $t(18)
              )
            }
          }
      ...
      const h = t?.map((g) => g.originDenom.includes("native") || !g.originDenom || g.originDenom === Kc ? {
        address: u,
        abi: Yc,
  ...
  const { inputState: r } = Ft(), s = zc(r.tab).find((l) => l.chainId === e), i = Nc(r.tab);
  t = t || i?.[e.toString()] || [];
  const u = Xc({
    chainId: e,
    assets: t,
...
fn.toByteArray = nu;
fn.fromByteArray = su;
var Ue = [], xe = [], $c = typeof Uint8Array < "u" ? Uint8Array : Array, Sn = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
for (var yt = 0, _c = Sn.length; yt < _c; ++yt)
  Ue[yt] = Sn[yt], xe[Sn.charCodeAt(yt)] = yt;
xe[45] = 62;
xe[95] = 63;
function is(e) {
  var t = e.length;
...
function nu(e) {
  var t, n = is(e), r = n[0], o = n[1], s = new $c(tu(e, r, o)), i = 0, u = o > 0 ? r - 4 : r, d;
  for (d = 0; d < u; d += 4)
...
(function(e) {
  ...
  l.prototype.swap16 = function() {
    ...
    let E = C - w, G = p - c;
    const _ = Math.min(E, G), H = this.slice(w, C), ee = a.slice(c, p);
    for (let K = 0; K < _; ++K)
      if (H[K] !== ee[K]) {
        E = H[K], G = ee[K];
        break;
  ...
  function L(f, a, c, p, w) {
    ...
    function _(ee, K) {
      return C === 1 ? ee[K] : ee.readUInt16BE(K * C);
    }
    let H;
    if (w) {
      let ee = -1;
      for (H = c; H < E; H++)
        if (_(f, H) === _(a, ee === -1 ? 0 : H - ee)) {
          if (ee === -1 && (ee = H), H - ee + 1 === G) return ee * C;
        } else
          ee !== -1 && (H -= H - ee), ee = -1;
    } else
  ...
  function Y(f, a, c, p) {
    return Yt(In(a, f.length - c), f, c, p);
  }
  function $(f, a, c, p) {
    return Yt(Li(a), f, c, p);
  }
  function O(f, a, c, p) {
    return Yt(_r(a), f, c, p);
  }
  function le(f, a, c, p) {
    return Yt(Oi(a, f.length - c), f, c, p);
  }
      ...
      if (w + G <= c) {
        ...
        switch (G) {
          case 1:
            ...
            break;
          case 2:
            _ = f[w + 1], (_ & 192) === 128 && (K = (C & 31) << 6 | _ & 63, K > 127 && (E = K));
            break;
          case 3:
            _ = f[w + 1], H = f[w + 2], (_ & 192) === 128 && (H & 192) === 128 && (K = (C & 15) << 12 | (_ & 63) << 6 | H & 63, K > 2047 && (K < 55296 || K > 57343) && (E = K));
            break;
          case 4:
            _ = f[w + 1], H = f[w + 2], ee = f[w + 3], (_ & 192) === 128 && (H & 192) === 128 && (ee & 192) === 128 && (K = (C & 15) << 18 | (_ & 63) << 12 | (H & 63) << 6 | ee & 63, K > 65535 && K < 1114112 && (E = K));
        }
      }
  ...
  function Pi(f) {
    if (f = f.split("=")[0], f = f.trim().replace(Di, ""), f.length < 2) return "";
    for (; f.length % 4 !== 0; )
      f = f + "=";
    return f;
  }
...
const iu = ss.Buffer;
function au() {
  const { address: e, isConnected: t, connector: n } = Zt(), r = !!(e && t && n);
  return R(
    () => ({
      protocol: D.Ethereum,
...
function cu() {
  const { publicKey: e, connected: t, wallet: n } = Jt(), r = !!(e && n && t), o = e?.toBase58();
  return R(
    () => ({
      protocol: D.Sealevel,
...
const Tr = (e) => Ce({
  ...
  return Ce({
    ...
    queryFn: async () => {
      ...
      return !d || !l || !x || !b ? null : s.validateTransfer({
        ...
        sender: l,
        recipient: b,
        originTokenAmount: new aa(v, x),
        senderPubKey: y
      });
    }
  });
}, ke = ({ size: e = "40", ...t }) => /* @__PURE__ */ T(
  "svg",
  {
    width: e,
    height: e,
  ...
  const { data: i } = Tr(t?.coingeckoId), { data: u } = us({
    chainId: n?.key,
    tokenAddress: t?.key
  }), d = R(() => i ?? u, [u, i]), h = R(() => {
    if (!r || !d || !t) return 0;
  ...
  return /* @__PURE__ */ T("div", { className: "flex items-center gap-1", children: [
    ...
    e && /* @__PURE__ */ T(
      oe,
      {
        ...
        onClick: l,
        children: [
          /* @__PURE__ */ m(Aa, { size: 12, className: "!size-3 text-foreground" }),
...
const mu = ({
  ...
  return d ? /* @__PURE__ */ T("div", { className: "flex items-center gap-1.5", children: [
    /* @__PURE__ */ T(
      "div",
      {
        className: se(
        ...
        children: [
          ...
          /* @__PURE__ */ m(vt, { mode: "wait", children: y ? /* @__PURE__ */ m(Je, { className: "inline-block h-4 w-14" }) : b && h ? /* @__PURE__ */ T(
            Ee.p,
            {
              initial: "hidden",
              animate: "visible",
...
function Yn(e) {
  ...
  return /* @__PURE__ */ T("div", { className: "bg-card-foreground flex w-full flex-col rounded-xl px-4 py-6", children: [
    /* @__PURE__ */ m("div", { className: "text-muted-foreground font-medium mb-3 text-xs", children: e.label }),
    /* @__PURE__ */ T("div", { className: "flex w-full items-end justify-between gap-2", children: [
      /* @__PURE__ */ T("div", { className: "flex flex-1 items-center gap-1", children: [
      ...
      e.isLoading ? /* @__PURE__ */ m(Je, { className: "h-9 w-22 rounded-full" }) : /* @__PURE__ */ T(
        "button",
        {
          className: se(
          ...
          children: [
            e.assetDetails ? /* @__PURE__ */ T(Te, { className: "mr-1 size-6 shrink-0", children: [
              /* @__PURE__ */ m(
                Se,
                {
    ...
    /* @__PURE__ */ T("div", { className: "text-muted-foreground mt-2 flex items-center justify-between text-xs", children: [
      /* @__PURE__ */ m(
        ls,
        {
...
const Kn = ({
  ...
  }, []), /* @__PURE__ */ T("span", { className: t, children: [
    e,
...
function _t(e) {
  return /* @__PURE__ */ T(
    "button",
    {
      ...
      onClick: e.onClick,
      children: [
        /* @__PURE__ */ m(vt, { mode: "wait", children: /* @__PURE__ */ m(Te, { className: "h-6 w-6 rounded-full", asChild: !0, children: /* @__PURE__ */ T(
          Ee.div,
          {
            initial: "initial",
            animate: "animate",
            exit: "exit",
            variants: e.selectType === "source" ? ho : mo,
            transition: De,
            children: [
              /* @__PURE__ */ m(
                Se,
        ...
        /* @__PURE__ */ m("div", { className: "flex max-w-full flex-col overflow-hidden", children: /* @__PURE__ */ m(vt, { mode: "wait", children: /* @__PURE__ */ m(
          Ee.span,
          {
            ...
            animate: "animate",
            exit: "exit",
            variants: e.selectType === "source" ? mo : ho,
            transition: De,
...
const fs = (e) => {
  ...
  }, [e.defaultTab, r]), o ? null : /* @__PURE__ */ T("div", { className: "mb-6 flex items-center gap-3", children: [
    /* @__PURE__ */ T(gu, { htmlFor: "airplane-mode", className: "flex items-center gap-1", children: [
      /* @__PURE__ */ m(ya, { size: 16, weight: "fill" }),
  ...
  return /* @__PURE__ */ T("div", { className: "flex items-center gap-1", children: [
    /* @__PURE__ */ m(Ca, { className: "size-4" }),
  ...
  id: "gas-info",
  children: /* @__PURE__ */ T("span", { className: "flex gap-1", children: [
    e.gasInfo.gasFees.map((t, n) => /* @__PURE__ */ T("span", { children: [
      t.amount,
      " ",
      t.symbol
    ] }, n)),
  ...
  const t = R(() => new ne(e.toAmount).dividedBy(e.fromAmount), [
    // fromTokenUsdPrice,
    // toTokenUsdPrice,
    e.fromAmount,
    e.toAmount
  ...
  const { inputState: r } = Ft(), o = R(() => r?.tab === pe.FAST ? /* @__PURE__ */ T(ye, { children: [
  ...
  return /* @__PURE__ */ m("div", { className: "bg-secondary mb-3 rounded-2xl p-4 text-xs text-foreground", children: /* @__PURE__ */ T("div", { className: "flex flex-col gap-3", children: [
  ...
  return /* @__PURE__ */ T("div", { className: "flex w-full items-center gap-4 overflow-hidden text-foreground", children: [
    /* @__PURE__ */ T("div", { className: "relative", children: [
      /* @__PURE__ */ T(Te, { className: "size-12", children: [
        /* @__PURE__ */ m(Se, { src: e?.logoURI }),
        /* @__PURE__ */ m(Be, { children: /* @__PURE__ */ m(ke, { className: "size-12" }) })
      ] }),
      /* @__PURE__ */ T(Te, { className: "absolute right-0 bottom-0 size-4 rounded-full", children: [
        /* @__PURE__ */ m(Se, { src: t?.logoURI }),
        /* @__PURE__ */ m(Be, { children: /* @__PURE__ */ m(ke, { className: "size-4" }) })
      ] })
    ] }),
    /* @__PURE__ */ T("div", { className: "flex flex-col overflow-hidden", children: [
      /* @__PURE__ */ m("span", { className: "text-3.5xl truncate leading-10.75", children: o }),
      t?.displayName && /* @__PURE__ */ T("span", { className: "text-secondary-foreground text-sm font-medium", children: [
        "on",
        " ",
...
const Bu = () => {
  ...
  return X(() => {
    t(g);
  }, [g, t]), /* @__PURE__ */ T("div", { className: "flex flex-col gap-1", children: [
    /* @__PURE__ */ T(
      "label",
      {
    ...
    s && /* @__PURE__ */ T(ye, { children: [
      ...
      !u && i && /* @__PURE__ */ T("div", { className: "text-foreground-2 bg-background rounded-md px-3 py-2 text-sm", children: [
...
}, Nu = (e) => e.inputAmount ? e.isInsufficientBalance ? "Insufficient balance" : e.isGasLoading ? /* @__PURE__ */ m(Kn, { text: "Fetching quote" }) : e.isValidating ? /* @__PURE__ */ m(Kn, { text: "Validating" }) : e.validation ? Object.values(e.validation)[0] : e.validationError ? "Insufficient funds for gas fee" : `Send to ${e.chainDisplayName}` : "Enter amount", go = ({
  className: e,
  onNavigate: t,
  ...
  const r = W((O) => O.state.from.chain), o = W((O) => O.state.to.chain), s = W((O) => O.state.from.token), i = W((O) => O.state.from.amount), u = W(
  ...
  }, [A, s?.key, b]), { data: B } = Tr(s?.coingeckoId), M = R(() => {
    if (!i) return "0";
    let O = new ne(i);
    if (u === Q.FIAT) {
      if (!B) return "0";
      O = new ne(i).dividedBy(B);
    }
    return O.toString();
  }, [i, B, u]), S = R(() => !M || !F ? !1 : new ne(M).isGreaterThan(F), [M, F]), { isFetching: U, gasInfo: z } = as(
    {
      originChainId: r?.key,
    ...
    validation: P,
    validationError: j,
    isInsufficientBalance: S
  });
  return /* @__PURE__ */ T(ye, { children: [
    /* @__PURE__ */ m(fs, {}),
    /* @__PURE__ */ T("div", { className: "relative mb-5 flex items-center gap-3", children: [
      /* @__PURE__ */ m(
        _t,
        {
          title: "From",
      ...
      /* @__PURE__ */ m("div", { className: "bg-card absolute top-1/2 left-1/2 z-10 -translate-x-1/2 -translate-y-1/2 rounded-full direction-button-bg", children: /* @__PURE__ */ m(
        oe,
        {
          ...
          onClick: () => d(),
          children: /* @__PURE__ */ m(jo, { className: "transition-transform duration-300 group-hover:scale-110 group-hover:rotate-180 group-disabled:opacity-50" })
        }
      ) }),
      /* @__PURE__ */ m(
        _t,
    ...
    /* @__PURE__ */ T("div", { className: "relative mb-3 flex flex-col gap-2", children: [
      /* @__PURE__ */ m(
        Yn,
        {
          label: "You send",
      ...
      /* @__PURE__ */ T("div", { className: "bg-secondary flex w-full flex-col rounded-xl px-4 py-6", children: [
    ...
    /* @__PURE__ */ m(Bu, {}),
    Y && z ? /* @__PURE__ */ m(pn, { gasInfo: z, isGasLoading: U, children: /* @__PURE__ */ m(_o, { fromToken: s }) }) : null,
    /* @__PURE__ */ m(
      ds,
      {
        className: "mt-auto",
        ...
        children: /* @__PURE__ */ m(
          oe,
          {
            className: se(
              ...
              (S || P || j) && !U ? "!bg-destructive/90 text-white" : ""
            ),
            size: "lg",
            onClick: () => t("review"),
            disabled: U || I || S || !s || !Y || !v?.chainType || !!P || !!j || L,
            children: $
          }
        )
      }
  ...
  failUnconfirmedTransfers: () => {
    e((t) => ({
      transfers: t.transfers.map(
        (n) => Ac.includes(n.status) ? n : { ...n, status: te.Failed }
      )
    }));
  },
...
function Ru(e) {
  const { connection: t } = Ra(), { wallet: n } = Jt(), r = t?.rpcEndpoint;
  return R(() => {
    try {
      const o = new URL(r).hostname, s = mc(e, o);
      return s ? {
...
`);
    super(i, n.cause ? { cause: n.cause } : void 0), Object.defineProperty(this, "details", {
      ...
      writable: !0,
      value: "BaseError"
    }), this.details = r, this.docsPath = o, this.metaMessages = n.metaMessages, this.name = n.name ?? this.name, this.shortMessage = t, this.version = ps;
  }
  walk(t) {
    return gs(this, t);
  }
...
function gs(e, t) {
  return t?.(e) ? e : e && typeof e == "object" && "cause" in e && e.cause !== void 0 ? gs(e.cause, t) : t ? null : e;
}
class ys extends N {
...
function Nr(e, t = {}) {
  return typeof e == "number" || typeof e == "bigint" ? Vu(e, t) : typeof e == "boolean" ? Gu(e, t) : nt(e) ? je(e, t) : xs(e, t);
}
function Gu(e, t = {}) {
...
function je(e, t = {}) {
  ...
  for (let i = 0, u = 0; i < o; i++) {
    const d = bo(r.charCodeAt(u++)), h = bo(r.charCodeAt(u++));
...
function Hn(e, t = {}) {
  return typeof e == "number" || typeof e == "bigint" ? J(e, t) : typeof e == "string" ? Cs(e, t) : typeof e == "boolean" ? As(e, t) : be(e, t);
}
function As(e, t = {}) {
  const n = `0x${Number(e)}`;
...
const Qt = /* @__PURE__ */ BigInt(2 ** 32 - 1), xo = /* @__PURE__ */ BigInt(32);
function sl(e, t = !1) {
  return t ? { h: Number(e & Qt), l: Number(e >> xo & Qt) } : { h: Number(e >> xo & Qt) | 0, l: Number(e & Qt) | 0 };
}
function il(e, t = !1) {
...
const Ao = new Uint8Array(new Uint32Array([287454020]).buffer)[0] === 68;
function fl(e) {
...
const ks = [], Ts = [], Ss = [], ml = /* @__PURE__ */ BigInt(0), Ut = /* @__PURE__ */ BigInt(1), pl = /* @__PURE__ */ BigInt(2), gl = /* @__PURE__ */ BigInt(7), yl = /* @__PURE__ */ BigInt(256), bl = /* @__PURE__ */ BigInt(113);
for (let e = 0, t = Ut, n = 1, r = 0; e < 24; e++) {
  [n, r] = [r, (2 * n + 3 * r) % 5], ks.push(2 * (5 * r + n)), Ts.push((e + 1) * (e + 2) / 2 % 64);
  let o = ml;
  for (let s = 0; s < 7; s++)
    t = (t << Ut ^ (t >> gl) * bl) % yl, t & pl && (o ^= Ut << (Ut << /* @__PURE__ */ BigInt(s)) - Ut);
  Ss.push(o);
}
const [wl, xl] = /* @__PURE__ */ il(Ss, !0), vo = (e, t, n) => n > 32 ? ul(e, t, n) : al(e, t, n), Io = (e, t, n) => n > 32 ? ll(e, t, n) : cl(e, t, n);
function Al(e, t = 24) {
  ...
  for (let r = 24 - t; r < 24; r++) {
    ...
    for (let i = 0; i < 10; i += 2) {
      const u = (i + 8) % 10, d = (i + 2) % 10, h = n[d], l = n[d + 1], y = vo(h, l, 1) ^ n[u], g = Io(h, l, 1) ^ n[u + 1];
      for (let b = 0; b < 50; b += 10)
        e[i + b] ^= y, e[i + b + 1] ^= g;
    }
    let o = e[2], s = e[3];
    for (let i = 0; i < 24; i++) {
      const u = Ts[i], d = vo(o, s, u), h = Io(o, s, u), l = ks[i];
...
class zr extends Is {
  ...
  keccak() {
    Ao || Co(this.state32), Al(this.state32, this.rounds), Ao || Co(this.state32), this.posOut = 0, this.pos = 0;
  }
  update(t) {
    nn(this);
...
const Cl = (e, t, n) => Es(() => new zr(t, e, n)), vl = /* @__PURE__ */ Cl(1, 136, 256 / 8);
function bn(e, t) {
...
function wn(e, t) {
  if (Fn.has(`${e}.${t}`))
    return Fn.get(`${e}.${t}`);
  const n = e.substring(2).toLowerCase(), r = bn(xs(n), "bytes"), o = n.split("");
...
const Tl = /^(u?int)(8|16|24|32|40|48|56|64|72|80|88|96|104|112|120|128|136|144|152|160|168|176|184|192|200|208|216|224|232|240|248|256)?$/;
function Rs(e, t) {
...
function Ur({ param: e, value: t }) {
  ...
  if (e.type.startsWith("uint") || e.type.startsWith("int")) {
    const r = e.type.startsWith("int"), [, , o = "256"] = Tl.exec(e.type) ?? [];
    return Rl(t, {
      signed: r,
      size: Number(o)
    });
...
function Ml(e) {
  ...
  return { dynamic: !1, encoded: et(As(e)) };
}
...
function zl(e) {
  const t = Cs(e), n = Math.ceil(fe(t) / 32), r = [];
  for (let o = 0; o < n; o++)
    r.push(et(Bs(t, o * 32, (o + 1) * 32), {
      dir: "right"
    }));
  return {
    dynamic: !0,
...
function Ul(e, { param: t }) {
  ...
  return {
    dynamic: n,
    encoded: n ? Dr(r) : Tt(r.map(({ encoded: o }) => o))
  };
}
function Pr(e) {
  const t = e.match(/^(.*)\[(\d+)?\]$/);
...
function zs(e) {
  ...
  if (!("inputs" in o))
    throw new wo({ docsPath: Rn });
  if (!o.inputs || o.inputs.length === 0)
    throw new wo({ docsPath: Rn });
  const s = Rs(o.inputs, n);
  return gn([r, s]);
}
function mt(e) {
...
async function Ll({ hash: e, signature: t }) {
  const n = nt(e) ? e : Hn(e), { secp256k1: r } = await import("./secp256k1-7RQEduL5.js");
  return `0x${(() => {
    if (typeof t == "object" && "r" in t && "s" in t) {
      const { r: h, s: l, v: y, yParity: g } = t, b = Number(g ?? y), x = Eo(b);
      return new r.Signature(kt(h), kt(l)).addRecoveryBit(x);
    }
    const i = nt(t) ? t : Hn(t), u = ft(`0x${i.slice(130)}`), d = Eo(u);
...
function Zl(e, t = "hex") {
  const n = Us(e), r = Lr(new Uint8Array(n.length));
  return n.encode(r), t === "hex" ? be(r.bytes) : r.bytes;
}
function Us(e) {
  return Array.isArray(e) ? Jl(e.map((t) => Us(t))) : ql(e);
}
function Jl(e) {
  const t = e.reduce((o, s) => o + s.length, 0), n = Ds(t);
  return {
...
function ql(e) {
  const t = typeof e == "string" ? je(e) : e, n = Ds(t.length);
  return {
...
function Cn(e) {
  ...
  if (i && !rt(i.address))
    throw new Gt({ address: i.address });
  if (s && !rt(s))
    throw new Gt({ address: s });
...
function ud(e) {
  const t = (e.transactions ?? []).map((n) => typeof n == "string" ? n : qs(n));
  return {
    ...e,
...
async function Vt(e, { blockHash: t, blockNumber: n, blockTag: r, includeTransactions: o } = {}) {
  ...
  if (t ? d = await e.request({
    ...
    throw new Zs({ blockHash: t, blockNumber: n });
...
async function ld(e, t) {
  ...
  try {
    ...
    if (typeof s == "function") {
      const u = n || await ue(e, Vt, "getBlock")({}), d = await s({
        block: u,
        client: e,
        request: o
      });
    ...
    const [s, i] = await Promise.all([
      n ? Promise.resolve(n) : ue(e, Vt, "getBlock")({}),
      ue(e, js, "getGasPrice")({})
    ]);
...
async function To(e, t) {
  ...
  if (i < 1)
    throw new id();
  const d = 10 ** (i.toString().split(".")[1]?.length ?? 0), h = (g) => g * BigInt(Math.ceil(i * d)) / BigInt(d), l = n || await ue(e, Vt, "getBlock")({});
  ...
  return {
    gasPrice: o?.gasPrice ?? h(await ue(e, js, "getGasPrice")({}))
  };
}
class dd extends N {
  constructor(t, { account: n, docsPath: r, chain: o, data: s, gas: i, gasPrice: u, maxFeePerGas: d, maxPriorityFeePerGas: h, nonce: l, to: y, value: g }) {
...
`), r.state && (o += `      state:
`, o += So(r.state)), r.stateDiff && (o += `      stateDiff:
`, o += So(r.stateDiff)), o;
  }, `  State Override:
`).slice(0, -1);
}
...
function gd(e) {
  ...
  if (s !== void 0 && (i.code = s), t !== void 0 && (i.balance = J(t)), n !== void 0 && (i.nonce = J(n)), r !== void 0 && (i.state = Bo(r)), o !== void 0) {
    if (i.state)
      throw new md();
    i.stateDiff = Bo(o);
  }
...
async function Ks(e, t) {
  ...
  try {
    let j = function($) {
      const { block: O, request: le, rpcStateOverride: ae } = $;
      return e.request({
        method: "eth_estimateGas",
        params: ae ? [le, O ?? "latest", ae] : O ? [le, O] : [le]
      });
    };
    const { accessList: o, authorizationList: s, blobs: i, blobVersionedHashes: u, blockNumber: d, blockTag: h, data: l, gas: y, gasPrice: g, maxFeePerBlobGas: b, maxFeePerGas: x, maxPriorityFeePerGas: v, nonce: k, value: A, stateOverride: I, ...F } = await ei(e, {
      ...t,
      parameters: (
    ...
    }), M = (d ? J(d) : void 0) || h, S = Ys(I), U = await (async () => {
      if (F.to)
        return F.to;
      if (s && s.length > 0)
        return await Ps({
    ...
    let Y = BigInt(await j({ block: M, request: L, rpcStateOverride: S }));
    if (s) {
      const $ = await yd(e, { address: L.from }), O = await Promise.all(s.map(async (le) => {
...
class Cd extends Is {
  ...
  digestInto(t) {
    nn(this), vs(t, this), this.finished = !0;
...
const No = 6, Xs = 32, Jr = 4096, $s = Xs * Jr, Fo = $s * No - // terminator byte (0x80).
...
function Fd(e) {
  ...
  let o = !0, s = 0;
  for (; o; ) {
    const i = Lr(new Uint8Array($s));
    let u = 0;
    for (; u < Jr; ) {
      const d = t.slice(s, s + (Xs - 1));
...
function Md(e) {
  const { data: t, kzg: n, to: r } = e, o = e.blobs ?? Fd({ data: t }), s = e.commitments ?? Qs({ blobs: o, kzg: n, to: r }), i = e.proofs ?? Hs({ blobs: o, commitments: s, kzg: n, to: r }), u = [];
...
async function ei(e, t) {
  const { account: n = e.account, blobs: r, chain: o, gas: s, kzg: i, nonce: u, nonceManager: d, parameters: h = _s, type: l } = t, y = n && mt(n), g = { ...t, ...y ? { from: y?.address } : {} };
  let b;
  async function x() {
    return b || (b = await ue(e, Vt, "getBlock")({ blockTag: "latest" }), b);
  }
  let v;
  async function k() {
    return v || (o ? o.id : typeof t.chainId < "u" ? t.chainId : (v = await ue(e, Ws, "getChainId")({}), v));
  }
  if ((h.includes("blobVersionedHashes") || h.includes("sidecars")) && r && i) {
    const A = Qs({ blobs: r, kzg: i });
    ...
    if (h.includes("sidecars")) {
      const I = Hs({ blobs: r, commitments: A, kzg: i }), F = Md({
        blobs: r,
        commitments: A,
        proofs: I,
        to: "hex"
  ...
  if (h.includes("chainId") && (g.chainId = await k()), (h.includes("fees") || h.includes("type")) && typeof l > "u")
    try {
      ...
      let A = Mo.get(e.uid);
      typeof A > "u" && (A = typeof (await x())?.baseFeePerGas == "bigint", Mo.set(e.uid, A)), g.type = A ? "eip1559" : "legacy";
    }
  if (h.includes("fees"))
    if (g.type !== "legacy" && g.type !== "eip2930") {
      if (typeof g.maxFeePerGas > "u" || typeof g.maxPriorityFeePerGas > "u") {
        const A = await x(), { maxFeePerGas: I, maxPriorityFeePerGas: F } = await To(e, {
          block: A,
          chain: o,
          request: g
        });
      ...
      if (typeof t.gasPrice > "u") {
        const A = await x(), { gasPrice: I } = await To(e, {
          block: A,
          chain: o,
          request: g,
          type: "legacy"
  ...
  if (h.includes("gas") && typeof s > "u" && (g.gas = await ue(e, Ks, "estimateGas")({
    ...g,
...
async function Ud(e, t) {
  ...
  try {
    ...
    if (k?.type === "json-rpc" || k === null) {
      let I;
      r !== null && (I = await ue(e, Ws, "getChainId")({}), Ql({
        currentChainId: I,
        chain: r
      }));
      const F = e.chain?.formatters?.transactionRequest?.format, M = (F || Wr)({
        ...
        to: A,
        value: x
      }), S = Un.get(e.uid), U = S ? "wallet_sendTransaction" : "eth_sendTransaction";
      try {
    ...
    if (k?.type === "local") {
      const I = await ue(e, ei, "prepareTransactionRequest")({
        account: k,
        accessList: o,
        authorizationList: s,
        blobs: i,
...
function Gd(e) {
  ...
  return [wn(be(Ms(t, -20))), 32];
}
function Vd(e, t, { length: n, staticPosition: r }) {
  if (!n) {
    const i = qe(e.readBytes(cr)), u = r + i, d = u + zo;
    e.setPosition(u);
    const h = qe(e.readBytes(zo)), l = Wt(t);
    let y = 0;
    const g = [];
    for (let b = 0; b < h; ++b) {
...
function Wt(e) {
  ...
  if (t === "string" || t === "bytes" || t.endsWith("[]"))
    return !0;
  if (t === "tuple")
    return e.components?.some(Wt);
  const n = Pr(e.type);
  return !!(n && Wt({ ...e, type: n[1] }));
}
const Yd = (e) => bn(Nr(e));
function Kd(e) {
  return Yd(e);
...
const ti = /^bytes([1-9]|1[0-9]|2[0-9]|3[0-2])?$/, ni = /^u?int(8|16|24|32|40|48|56|64|72|80|88|96|104|112|120|128|136|144|152|160|168|176|184|192|200|208|216|224|232|240|248|256)?$/, ri = /^\(.+?\).*?$/, Uo = /^tuple(?<array>(\[(\d*)\])*)$/;
function lr(e) {
  let t = e.type;
  if (Uo.test(e.type) && "components" in e) {
    t = "(";
...
const oi = /^error (?<name>[a-zA-Z$_][a-zA-Z0-9$_]*)\((?<parameters>.*?)\)$/;
function Xd(e) {
  return oi.test(e);
}
function $d(e) {
...
const si = /^event (?<name>[a-zA-Z$_][a-zA-Z0-9$_]*)\((?<parameters>.*?)\)$/;
function _d(e) {
  return si.test(e);
}
function ef(e) {
...
const ii = /^function (?<name>[a-zA-Z$_][a-zA-Z0-9$_]*)\((?<parameters>.*?)\)(?: (?<scope>external|public{1}))?(?: (?<stateMutability>pure|view|nonpayable|payable{1}))?(?: returns\s?\((?<returns>.*?)\))?$/;
function tf(e) {
  return ii.test(e);
}
function nf(e) {
...
const ai = /^struct (?<name>[a-zA-Z$_][a-zA-Z0-9$_]*) \{(?<properties>.*?)\}$/;
function ci(e) {
  return ai.test(e);
}
function rf(e) {
...
const ui = /^constructor\((?<parameters>.*?)\)(?:\s(?<stateMutability>payable{1}))?$/;
function of(e) {
  return ui.test(e);
}
function sf(e) {
...
const li = /^fallback\(\) external(?:\s(?<stateMutability>payable{1}))?$/;
function af(e) {
  return li.test(e);
}
function cf(e) {
...
function ht(e, t) {
  ...
  if (Dn.has(n))
    return Dn.get(n);
  const r = ri.test(e), o = Ke(r ? Ff : Nf, e);
  if (!o)
  ...
  if (r) {
    ...
    d = `${o.type}256`;
  else if (d = o.type, t?.type !== "struct" && !di(d))
    throw new hf({ type: d });
  if (o.modifier) {
...
function di(e) {
  return e === "address" || e === "bool" || e === "function" || e === "string" || ti.test(e) || ni.test(e);
}
...
function zf(e) {
  return e === "address" || e === "bool" || e === "function" || e === "string" || e === "tuple" || ti.test(e) || ni.test(e) || Rf.test(e);
}
function Uf(e, t) {
    ...
    if (ri.test(i.type))
      ...
      if (l in t) {
        ...
        r.push({
          ...i,
          type: `tuple${h ?? ""}`,
          components: fi(t[l] ?? [], t, /* @__PURE__ */ new Set([...n, l]))
        });
      } else if (di(l))
        r.push(i);
      else
...
const Gf = mi, pi = (e) => Bs(mi(e), 0, 4);
function gi(e) {
  const { abi: t, args: n = [], name: r } = e, o = nt(r, { strict: !1 }), s = t.filter((u) => o ? u.type === "function" ? pi(u) === r : u.type === "event" ? Gf(u) === r : !1 : "name" in u && u.name === r);
  if (s.length === 0)
    return;
  if (s.length === 1)
    return s[0];
  ...
  for (const u of s) {
    ...
    if (n.every((h, l) => {
      ...
      if (i && "inputs" in i && i.inputs) {
        const h = yi(u.inputs, i.inputs, n);
        if (h)
          throw new $u({
            abiItem: u,
            type: h[0]
...
function yi(e, t, n) {
  for (const r in e) {
    ...
    if (o.type === "tuple" && s.type === "tuple" && "components" in o && "components" in s)
      return yi(o.components, s.components, n[r]);
...
function Vf(e) {
  ...
  let s = t[0];
  if (r) {
    const u = gi({ abi: t, args: n, name: r });
    if (!u)
...
const Do = "/docs/contract/encodeFunctionData";
function Wf(e) {
  ...
  if (r) {
    ...
    if (!s)
      throw new en(r, { docsPath: Do });
    o = s;
  }
  if (o.type !== "function")
    throw new en(void 0, { docsPath: Do });
  return {
    abi: [o],
    functionName: pi(Xn(o))
  };
}
function Zf(e) {
  const { args: t } = e, { abi: n, functionName: r } = e.abi.length === 1 && e.functionName?.startsWith("0x") ? e : Wf(e), o = n[0], s = r, i = "inputs" in o && o.inputs ? Rs(o.inputs, t ?? []) : void 0;
  return gn([s, i ?? "0x"]);
}
...
function $f({ fn: e, id: t, shouldSplitBatch: n, wait: r = 0, sort: o }) {
  ...
  return {
    flush: i,
    async schedule(l) {
      const { promise: y, resolve: g, reject: b } = bi();
...
async function _f(e, t) {
  const { account: n = e.account, batch: r = !!e.batch?.multicall, blockNumber: o, blockTag: s = "latest", accessList: i, blobs: u, code: d, data: h, factory: l, factoryData: y, gas: g, gasPrice: b, maxFeePerBlobGas: x, maxFeePerGas: v, maxPriorityFeePerGas: k, nonce: A, to: I, value: F, stateOverride: B, ...M } = t, S = n ? mt(n) : void 0;
  if (d && (l || y))
  ...
  try {
    Cn(t);
    const Y = (o ? J(o) : void 0) || s, $ = Ys(B), O = e.chain?.formatters?.transactionRequest?.format, ae = (O || Wr)({
      // Pick out extra data that might exist on the chain's transaction request type.
      ...Vr(M, { format: O }),
      from: S?.address,
      accessList: i,
    ...
    if (r && eh({ request: ae }) && !$)
      try {
        ...
        if (!(Qe instanceof Ls) && !(Qe instanceof $n))
          throw Qe;
      }
    ...
    const Y = oh(j), { offchainLookup: $, offchainLookupSignature: O } = await import("./ccip-CSVNRA9-.js");
    if (e.ccipRead !== !1 && Y?.slice(0, 10) === O && I)
      return { data: await $(e, { data: Y, to: I }) };
...
function nh(e) {
  ...
  return zs({
    abi: hi(["constructor(bytes, bytes)"]),
    bytecode: Qf,
    args: [t, n]
  });
}
function rh(e) {
  ...
  return zs({
    abi: hi(["constructor(address, bytes, address, bytes)"]),
    bytecode: Hf,
    args: [o, t, n, r]
  });
}
...
async function wi(e, { blockHash: t, blockNumber: n, blockTag: r, hash: o, index: s }) {
  ...
  return (e.chain?.formatters?.transaction?.format || qs)(d);
}
const fh = {
  "0x0": "reverted",
  "0x1": "success"
...
const On = /* @__PURE__ */ new Map(), Lo = /* @__PURE__ */ new Map();
let mh = 0;
function hr(e, t, n) {
  ...
  const h = n(d);
  return typeof h == "function" && Lo.set(e, h), i;
}
async function mr(e) {
...
function Oo(e, { delay: t = 100, retryCount: n = 2, shouldRetry: r = () => !0 } = {}) {
...
async function yh(e, {
  ...
  let h, l, y, g = !1;
  const { promise: b, resolve: x, reject: v } = bi(), k = u ? setTimeout(() => v(new td({ hash: n })), u) : void 0, A = hr(d, { onReplaced: r, resolve: x, reject: v }, (I) => {
    const F = ue(e, gh, "watchBlockNumber")({
      ...
      async onBlockNumber(B) {
        ...
        if (!g)
          try {
            if (y) {
              if (t > 1 && (!y.blockNumber || S - y.blockNumber + 1n < t))
                return;
              M(() => I.resolve(y));
              return;
            }
            if (h || (g = !0, await Oo(async () => {
              h = await ue(e, wi, "getTransaction")({ hash: n }), h.blockNumber && (S = h.blockNumber);
            }, {
              delay: i,
              retryCount: s
            }), g = !1), y = await ue(e, Po, "getTransactionReceipt")({ hash: n }), t > 1 && (!y.blockNumber || S - y.blockNumber + 1n < t))
              return;
            M(() => I.resolve(y));
          } catch (U) {
            if (U instanceof Gs || U instanceof Vs) {
              if (!h) {
                g = !1;
                return;
              }
              try {
                l = h, g = !0;
                const z = await Oo(() => ue(e, Vt, "getBlock")({
                  blockNumber: S,
                  includeTransactions: !0
                }), {
                  delay: i,
                  retryCount: s,
                  shouldRetry: ({ error: j }) => j instanceof Zs
                });
                g = !1;
                const P = z.transactions.find(({ from: j, nonce: Y }) => j === l.from && Y === l.nonce);
                if (!P || (y = await ue(e, Po, "getTransactionReceipt")({
                  hash: P.hash
                }), t > 1 && (!y.blockNumber || S - y.blockNumber + 1n < t)))
                  return;
                let L = "replaced";
...
`), n.cause && (this.cause = n.cause), this.details = r, this.docsPath = o, this.metaMessages = n.metaMessages, this.shortMessage = t;
  ...
  walk(t) {
    return xi(this, sn, "m", Ai).call(this, this, t);
  }
}
sn = /* @__PURE__ */ new WeakSet(), Ai = function e(t, n) {
  return n?.(t) ? t : t.cause ? xi(this, sn, "m", e).call(this, t.cause, n) : t;
};
class xh extends ot {
  constructor() {
...
async function Eh(e, t) {
  ...
  const { connector: d } = Ci(e), h = await (async () => {
    if (!(!("data" in t) || !t.data) && !(o ?? d)?.supportsSimulation && s !== null)
      return s === void 0 ? Ot(u, Ks, "estimateGas")({
        ...i,
        account: n,
        chain: r ? { id: r } : null
      }) : s;
  })();
  return await Ot(u, Ud, "sendTransaction")({
    ...i,
    ...n ? { account: n } : {},
    gas: h,
    chain: r ? { id: r } : null
...
async function Sh(e, t) {
  const { chainId: n, timeout: r = 0, ...o } = t, s = e.getClient({ chainId: n }), u = await Ot(s, yh, "waitForTransactionReceipt")({ ...o, timeout: r });
  if (u.status === "reverted") {
    const h = await Ot(s, wi, "getTransaction")({ hash: u.transactionHash }), y = await Ot(s, _f, "call")({
      ...h,
    ...
    }), g = y?.data ? Sa(`0x${y.data.substring(138)}`) : "unknown reason";
    throw new Error(g);
  }
  return {
    ...u,
...
function Nh(e) {
  const t = Na(), n = ie(
    ...
    async ({
      ...
      q.debug("Checking wallet current chain"), Ci(t), q.debug(`Sending tx on chain ${i}`);
      const h = Bh(s.transaction), l = await Eh(t, {
        chainId: d,
        ...h
      });
      return { hash: l, confirm: () => Sh(t, {
        chainId: d,
        hash: l,
        confirmations: 1
      }).then((b) => ({
...
function Fh(e) {
  const { sendTransaction: t } = Jt(), n = ie(async (s) => {
...
function vi(e) {
  const t = gc(e), { cosmos: n } = ct(), { data: r, isLoading: o } = za({
    ...
    async ({
      ...
      if (g.type === we.CosmJsWasm) {
        ...
        const B = r[v.chainId], M = await Oa.connectWithSigner(
          v.rpcUrls[0].http,
          B?.offlineSignerAuto,
          {
        ...
        const B = r[v.chainId];
        A = await (await Ga.connectWithSigner(
          v.rpcUrls.map((S) => S.http),
          B?.offlineSignerAuto,
          {
            metadata: {
              // set zero gas price here so it does not error. actual gas price
...
async function Lh(e, t) {
  ...
  if (e.token?.decimals && !isNaN(parseFloat(t)) && e.token.coinGeckoId) {
    const r = parseFloat(t) / 10 ** (e.token?.decimals ?? 6), o = await cs(e.token.coinGeckoId);
...
const Oh = () => {
  ...
  return { logTx: ie(
    async (n) => {
      if (Wa.debug("useTxLogging", n), n.data === void 0)
        return;
      const {
        txHash: r,
        address: o,
      ...
      const A = await Lh(i, u), I = Za.parse(window.navigator.userAgent), F = I.platform.type?.replace(" ", "_")?.toLowerCase() ?? "unknown", B = I.browser.name?.replace(" ", "_")?.toLowerCase() ?? "unknown", M = `${F}_${B}`, S = {
        ...v,
        browser: M,
...
function Zh() {
  const { transfers: e, addTransfer: t, updateTransferStatus: n } = Sr(), r = e.length, o = Nt(), s = it(), i = hn(), u = Uu(o), d = Mh(o), { isLoading: h } = vi(o), { logTx: l } = Oh(), [y, g] = he(!1), b = ie(
    (x) => Kh({
      warpCore: s,
      values: x,
      transferIndex: r,
...
async function Jh(e) {
  ...
  q.debug("Preparing transfer transaction(s)"), s(r, te.Preparing);
...
async function jh(e, t) {
  const {
    ...
    sender: v,
    recipient: y
  }), F = s[b].sendTransaction, B = s[b].sendMultiTransaction, M = i.chains[b], S = [];
  let U;
  const z = {
    txHash: "",
    address: v,
  ...
  if (I.length > 1 && I.every((L) => L.type === we.Starknet)) {
    ...
    q.debug(`${Y} transaction confirmed, hash:`, L), S.push(L);
  } else
    for (const L of I) {
      u(o, Ht[L.category][0]);
      ...
      L.category === lt.Transfer && (z.txHash = j, d(z)), u(o, Ht[L.category][1]), U = await Y();
      const $ = Wn(L.category);
      q.debug(`${$} transaction confirmed, hash:`, j), S.push(j);
    }
  const P = U ? hc(k, h, U) : void 0;
  return { hashes: S, txReceipt: U, msgId: P };
}
function Yh(e, t, n, r, o, s) {
  const i = e instanceof Error ? e.message : String(e);
  r(n, te.Failed), q.error(`Error at stage ${t}`, { error: e, errorDetails: i }), i.includes(Gh) || (i.includes(Vh) || i.includes(Wh) ? q.error(
    "Transaction timed out",
    `${uc(o, s)} may be busy. Please try again.`
  ) : q.error(
...
async function Kh(e) {
  ...
  r(!0);
  const o = te.Preparing;
  try {
    const s = await Jh(e);
    await qh(e, s);
    const { hashes: i, msgId: u } = await jh(e, s);
    n(t, te.ConfirmedTransfer, {
      originTxHash: i.at(-1),
      msgId: u
    });
  } catch (s) {
...
const Qh = {
  [te.Preparing]: "Error while preparing the transactions.",
  [te.CreatingTxs]: "Error while creating the transactions.",
  [te.SigningApprove]: "Error while signing the approve transaction.",
  [te.ConfirmingApprove]: "Error while confirming the approve transaction.",
  [te.SigningTransfer]: "Error while signing the transfer transaction.",
  [te.ConfirmingTransfer]: "Error while confirming the transfer transaction."
}, Ht = {
  [lt.Approval]: [
    te.SigningApprove,
    te.ConfirmingApprove
  ],
  [lt.Revoke]: [
    te.SigningRevoke,
    te.ConfirmingRevoke
  ...
  const [o, s] = he(0);
  X(() => {
    if (r === te.ConfirmingApprove) {
      s((u) => u + 1);
      return;
    }
  }, [r]);
  const i = new Array(n).fill(0);
  return /* @__PURE__ */ m("div", { children: /* @__PURE__ */ T("div", { className: "flex flex-col items-center gap-3 text-center text-foreground", children: [
    /* @__PURE__ */ m("div", { className: "flex items-center justify-center gap-4", children: i.map((u, d) => d === o ? /* @__PURE__ */ m(
      ...
      d
    ) : d < o ? /* @__PURE__ */ m(ba, { className: "size-4" }, d) : /* @__PURE__ */ m(
      "div",
      {
...
const ki = (e) => /* @__PURE__ */ T("div", { className: "mb-5 text-foreground", children: [
  /* @__PURE__ */ T("div", { className: "mb-4 flex items-center gap-2", children: [
    /* @__PURE__ */ m(
      "button",
      {
        type: "button",
  ...
  /* @__PURE__ */ T("div", { className: "relative flex flex-col gap-1", children: [
    /* @__PURE__ */ m(
      Go,
      {
        title: "You pay",
...
function Go(e) {
  return /* @__PURE__ */ T("div", { className: "bg-card-foreground rounded-2xl p-4", children: [
    /* @__PURE__ */ m("div", { className: "text-muted-foreground text-xs", children: e.title }),
    /* @__PURE__ */ T("div", { className: "mt-1 flex items-center justify-between", children: [
      /* @__PURE__ */ T("div", { children: [
        /* @__PURE__ */ T("div", { className: "text-2xl font-semibold", children: [
          e.displayFormat === Q.FIAT ? "$" : "",
          e.amount,
          " ",
          e.displayFormat === Q.TOKEN ? e.token?.symbol : ""
        ] }),
        /* @__PURE__ */ T("div", { className: "mt-1 flex items-center gap-1", children: [
          /* @__PURE__ */ m(
            ls,
            {
          ...
          /* @__PURE__ */ T("div", { className: "flex gap-2", children: [
            ...
            /* @__PURE__ */ T("div", { className: "flex items-center gap-1", children: [
              /* @__PURE__ */ T(Te, { className: "size-4 rounded-full", children: [
                /* @__PURE__ */ m(Se, { src: e.chain?.logoURI }),
                /* @__PURE__ */ m(Be, { children: /* @__PURE__ */ m(ke, { className: "size-4" }) })
              ] }),
      ...
      /* @__PURE__ */ m("div", { className: "flex items-center gap-2 text-sm", children: /* @__PURE__ */ T("div", { className: "relative", children: [
        /* @__PURE__ */ T(Te, { className: "size-12", children: [
          /* @__PURE__ */ m(Se, { src: `${e.token?.logoURI}` }),
          /* @__PURE__ */ m(Be, { children: /* @__PURE__ */ m(ke, { className: "size-12" }) })
        ] }),
        /* @__PURE__ */ T(Te, { className: "absolute right-0 bottom-0 size-4 rounded-full", children: [
          /* @__PURE__ */ m(Se, { src: e.chain?.logoURI }),
...
const _h = ({
  ...
  ), i = W((A) => A.setScreen), u = Sr((A) => A.transfers), { data: d } = Tr(r?.coingeckoId), h = R(() => {
    if (!o) return "0";
    let A = new ne(o);
    if (s === Q.FIAT) {
      if (!d) return "0";
      A = new ne(o).dividedBy(d);
    }
    return A.toString();
  ...
  const { isFetching: v, gasInfo: k } = as(
    {
      originChainId: t?.key,
  ...
  return /* @__PURE__ */ T(ye, { children: [
    /* @__PURE__ */ T("div", { className: "flex-1", children: [
      ...
      /* @__PURE__ */ m(pn, { gasInfo: k, isGasLoading: v, children: /* @__PURE__ */ m(_o, { fromToken: r }) }),
  ...
  oe,
  {
    ...
    children: /* @__PURE__ */ T(
      Ee.button,
      {
        ...
        variants: em,
        children: [
          /* @__PURE__ */ T(Te, { className: "h-8 w-8 shrink-0 bg-white p-1", children: [
            /* @__PURE__ */ m(Se, { src: It(e.logoUri) ?? "" }),
            /* @__PURE__ */ m(Be, { children: /* @__PURE__ */ m(ke, {}) })
          ] }),
          /* @__PURE__ */ m("div", { className: "w-full text-left", children: /* @__PURE__ */ T("div", { className: "flex items-center justify-between", children: [
            /* @__PURE__ */ T("span", { className: "flex flex-col", children: [
  ...
  oe,
  {
    ...
    onClick: e.onSelect,
    children: [
      /* @__PURE__ */ T(Te, { className: "size-5 shrink-0 rounded-none", children: [
        /* @__PURE__ */ m(Se, { src: e.chain.logoURI }),
...
const Si = ({
  ...
  const [l, y] = ut.useState(""), [g, b] = ut.useState(""), x = ut.useMemo(() => {
    ...
    if (v?.length !== 0)
      return k ? v.sort((B, M) => {
        const S = k.balances[B.originDenom]?.amount;
        return k.balances[M.originDenom]?.amount?.isGreaterThan(S ?? 0) ? 1 : -1;
  ...
  return /* @__PURE__ */ T(ye, { children: [
    /* @__PURE__ */ T("div", { className: "mb-6 flex items-center gap-2", children: [
      /* @__PURE__ */ m(
        "button",
        {
          type: "button",
    ...
    /* @__PURE__ */ T("div", { className: "flex h-[554px] flex-col", children: [
      n && /* @__PURE__ */ T("div", { className: "flex-shrink-0", children: [
        /* @__PURE__ */ m(
          pr,
          {
      ...
      !h && /* @__PURE__ */ T(ye, { children: [
        /* @__PURE__ */ m("div", { className: "my-5 flex flex-shrink-0 justify-center", children: /* @__PURE__ */ m(Ei, { className: "rounded-md2 max-w-5 border-1 border-white" }) }),
        r && /* @__PURE__ */ T("div", { className: "flex min-h-0 flex-1 flex-col", children: [
          /* @__PURE__ */ m(
            pr,
            {
          ...
          /* @__PURE__ */ m("div", { className: "no-scrollbar flex min-h-0 flex-1 flex-col gap-3 overflow-y-auto", children: A ? Array.from({ length: 3 }).map((B, M) => /* @__PURE__ */ T(
            "div",
            {
  ...
  const n = W((k) => k.state.from.chain), r = W((k) => k.state.to.chain), o = W((k) => k.state.from.token), s = W((k) => k.state.to.token), { setScreen: i, setFromChain: u, setToChain: d, setFromToken: h } = W(), l = Pe(n?.key), y = Pe(r?.key), g = Ir(
    ...
    l,
    y
  ), b = ln(), x = Rc(void 0, !0), v = R(() => x && n?.key ? x(b, n.key) : b, [x, n?.key, b]);
  return X(() => {
    r?.key && g.length == 0 && d({
      key: v[0]?.chainId,
...
}, Bi = ({ recivedAmount: e, toChain: t, toToken: n, explorerLink: r }) => /* @__PURE__ */ T("div", { className: "my-auto flex flex-col items-center gap-3 rounded-xl px-5 py-6", children: [
  /* @__PURE__ */ m("div", { className: "text-sm font-semibold", children: "Txn Successful!" }),
  /* @__PURE__ */ T("div", { className: "relative mb-3", children: [
    /* @__PURE__ */ T(Te, { className: "size-24", children: [
      /* @__PURE__ */ m(Se, { src: `${n?.logoURI}` }),
      /* @__PURE__ */ m(Be, { children: /* @__PURE__ */ m(ke, { className: "size-24" }) })
    ] }),
    /* @__PURE__ */ T(Te, { className: "absolute right-0 bottom-0 size-8 rounded-full", children: [
      /* @__PURE__ */ m(Se, { src: t?.logoURI }),
      /* @__PURE__ */ m(Be, { children: /* @__PURE__ */ m(ke, { className: "size-8" }) })
    ] })
  ] }),
  /* @__PURE__ */ T("div", { className: "text-center", children: [
    /* @__PURE__ */ T("p", { className: "text-3.5xl font-bold", children: [
      " ",
      Ae(e),
      " ",
      n?.symbol
    ] }),
    /* @__PURE__ */ T("p", { className: "text-sm font-normal", children: [
      "Received on ",
      t?.displayName
    ] })
  ] }),
  ...
  const t = W((d) => d.setScreen), n = Sr((d) => d.transfers), r = W((d) => d.state.from.amount), o = W((d) => d.state.to.chain), s = W((d) => d.state.from.token), i = Nt(), u = R(() => {
  ...
  switch (X(() => {
    ...
    case "failure":
      return /* @__PURE__ */ m(Wc, { className: e });
    case "selector":
      return /* @__PURE__ */ m(
        im,
        {
...
const dm = () => {
  ...
  return X(() => {
    t(u);
  }, [u, t]), /* @__PURE__ */ T("div", { className: "flex flex-col gap-1 mb-3", children: [
    /* @__PURE__ */ T(
      "label",
      {
        htmlFor: "recipient-address",
...
}, fm = (e) => e.inputAmount ? e.isInsufficientBalance ? "Insufficient balance" : e.isGasLoading ? /* @__PURE__ */ m(Kn, { text: "Fetching quote" }) : e.quoteError ? "Amount too low" : `Swap to ${e.chainDisplayName}` : "Enter amount", Wo = ({
  className: e,
  onNavigate: t,
  onSetSelectorContext: n
}) => {
  const r = V((Z) => Z.state.from.chain), o = V((Z) => Z.state.to.chain), s = V((Z) => Z.state.from.token), i = V((Z) => Z.state.to.token), u = V((Z) => Z.state.from.amount), d = V(
  ...
  ), { switchChains: h, setFromAmount: l, setAmountDisplayFormat: y } = V(), { tokens: g, isLoading: b } = qt(), { data: x, isLoading: v } = at(), k = x?.chains, A = R(() => {
    if (!(!k || !r?.key))
    ...
    const Z = g[r.key];
    return Z ? Z.find((jt) => jt.originDenom === s?.key) : null;
  }, [s, g, r]), B = tt(A), M = mn(I), { data: S, isLoading: U } = dn(
    { chainId: r?.key },
    !!r?.key && !!B
  ), z = R(() => {
    if (!S || !s?.key) return "0";
    const Z = S.balances[s.key];
    return Z ? Z.amount : "0";
  }, [S, s?.key]), { data: P } = us(
    {
      chainId: r?.key,
    ...
    if (d === Q.FIAT) {
      if (!P) return "0";
      Z = new ne(u).dividedBy(P);
    }
    return Z.toString();
  }, [P, u, d]), j = R(() => !F || !L ? "0" : new ne(L).multipliedBy(
    new ne(10).pow(F.decimals)
  ).toFixed(0), [F, L]), Y = R(() => !L || !z ? !1 : new ne(L).isGreaterThan(z), [L, z]), {
    data: $,
    isLoading: O,
    error: le
  } = Fi(
  ...
  return /* @__PURE__ */ T(ye, { children: [
    /* @__PURE__ */ T("div", { className: "overflow-y-scroll max-h-[90vh]", children: [
      /* @__PURE__ */ m(fs, {}),
      /* @__PURE__ */ T("div", { className: "relative mb-5 flex items-center gap-3", children: [
        /* @__PURE__ */ m(
          _t,
      ...
      /* @__PURE__ */ T("div", { className: "relative mb-3 flex flex-col gap-1", children: [
        /* @__PURE__ */ m(
          Yn,
          {
            label: "You send",
      ...
      /* @__PURE__ */ m(dm, {}),
      $ && /* @__PURE__ */ m(pn, { gasInfo: Oe, isGasLoading: O, children: /* @__PURE__ */ m(
        hs,
        {
          fromChain: r,
          toChain: o,
...
const hm = (e) => {
  const [t, n] = ut.useState("idle"), { data: r } = at(), [o, s] = he(
    void 0
  ), i = r?.chains, u = e.quote?.steps.length || 0, { data: d } = Fa(), { connector: h, chain: l } = Zt(), { switchChainAsync: y } = Ma(), { wallet: g } = Jt(), [b, x] = he(), [v, k] = he(
    new Array(u).fill({
      status: Ve.IDLE
    })
  ), A = async () => {
    try {
      ...
      if (e.fromChain?.chainType === D.Sealevel && i) {
        ...
        xt(U?.chainId) === Number(cn.solana) ? S = "https://mainnet.helius-rpc.com/?api-key=5175da47-fc80-456d-81e2-81e6e7459f73" : U && (S = U?.rpcUrl);
        const z = new wr(S);
        return {
          wallet: Qa(
      ...
      throw new Error("Error getting transacting wallet client", S);
    }
  }, I = ie(
    (S, U) => {
      k((z) => {
  ...
  X(() => {
    if (!i) return;
    const S = 0, U = o?.currentStep ?? o?.steps[0];
    if (U) {
  ...
  return /* @__PURE__ */ T(ye, { children: [
    ...
    t === "pending" && /* @__PURE__ */ m("div", { className: "mt-auto w-full", children: /* @__PURE__ */ m(Ii, {}) }),
    t === "idle" && /* @__PURE__ */ m(
      oe,
      {
        className: "mt-auto w-full transition-all hover:scale-105",
  ...
  const e = V((S) => S.state.from.chain), t = V((S) => S.state.to.chain), n = V((S) => S.state.from.token), r = V((S) => S.state.to.token), o = V((S) => S.state.from.amount), s = V(
    (S) => S.state.amountDisplayFormat
  ), i = V((S) => S.setScreen), u = V((S) => S.setTxData), { tokens: d } = qt(), { data: h } = at(), l = h?.chains, y = R(() => {
    if (!(!l || !e?.key))
      return l.find((S) => S.chainId === e?.key);
  }, [l, e?.key]), g = R(() => {
    if (!(!l || !t?.key))
      return l.find((S) => S.chainId === t?.key);
  }, [l, t?.key]), b = R(() => {
    if (!n || !d || !e) return null;
    const S = d[e.key];
    return S ? S.find((U) => U.originDenom === n?.key) : null;
  }, [n, d, e]), x = tt(y), v = mn(g), k = R(() => !b || !o ? "0" : new ne(o).multipliedBy(
    new ne(10).pow(b.decimals)
  ).toFixed(0), [b, o]), { data: A, isLoading: I } = Fi(
    {
      sender: x || "",
      recipient: v || "",
      sourceChainId: e?.key,
    ...
    if (!A) return "";
    const { details: S } = A;
    return S.currencyOut.amountFormatted;
  }, [A]), B = Ni(A);
  return /* @__PURE__ */ m(
    hm,
    {
  ...
  const n = V((x) => x.state.from.chain), r = V((x) => x.state.to.chain), o = V((x) => x.state.from.token), s = V((x) => x.state.to.token), { setScreen: i, setFromChain: u, setToChain: d, setFromToken: h, setToToken: l } = V(), { tokens: y } = qt(), { data: g } = at(), b = g?.chains;
  ...
  const t = V((y) => y.state.screen), { setFromToken: n, setToToken: r, setScreen: o } = V(), [s, i] = he(null), { tokens: u } = qt(), { data: d } = at();
  X(() => {
    if (!u) return;
  ...
  const { address: g } = Zt(), { wallet: b } = Jt(), { setAddress: x, cosmos: v } = ct(), { data: k } = xr({
    multiChain: !0,
  ...
  return /* @__PURE__ */ T(Ha, { children: [
    t.tab === pe.FAST && /* @__PURE__ */ m(ym, {}),
    t.tab === pe.ADVANCED && /* @__PURE__ */ m(Pc, { children: /* @__PURE__ */ m(cm, { onStatusChange: e }) })
  ] });
}, tp = ({
  children: e,
  ...t
}) => /* @__PURE__ */ m(Bc, { ...t, children: e });
export {
  Yu as A,
  N as B,
  Is as H,
