...
import { chains as ChainsTS } from "../constants/chains";
...
export async function assembleChainMetadata(
  ...
  const mergedChainMetadata = mergeChainMetadataMap(
    registryChainMetadata,
    filesystemMetadata,
  );
  ...
  const rpcOverrides = z
    .record(RpcUrlSchema)
    .safeParse(
      parsedRpcOverridesResult.success && parsedRpcOverridesResult.data,
    );
