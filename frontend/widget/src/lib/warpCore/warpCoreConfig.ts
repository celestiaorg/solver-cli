...
import { warpRouteConfigs as tsWarpRoutes } from "./warpRoutes";
...
export async function assembleWarpCoreConfig(
  ...
  const filteredRegistryConfigMap = registryWarpRoutes;
  const filteredRegistryConfigValues = Object.values(filteredRegistryConfigMap);
  const filteredRegistryTokens = filteredRegistryConfigValues
    .map((c) => c.tokens)
    .flat();
  const filteredRegistryOptions = filteredRegistryConfigValues
    .map((c) => c.options)
    .flat();

  const combinedTokens = [...filteredRegistryTokens, ...tsConfig.tokens];
  const tokens = dedupeTokens(combinedTokens);

  const combinedOptions = [...filteredRegistryOptions, tsConfig.options];
...
function reduceOptions(
  ...
  return optionsList.reduce<WarpCoreConfig["options"]>((acc, o) => {
    ...
    for (const key of Object.keys(o)) {
      const typedKey = key as keyof WarpCoreConfig["options"];
      const accValue = acc[typedKey] as unknown[] | undefined;
      const oValue = o[typedKey] as unknown[] | undefined;
      (acc as Record<string, unknown>)[typedKey] = (accValue || []).concat(
        oValue || [],
      );
    }
    return acc;
